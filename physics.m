...
if self.startRobot == 1
    ...
    AnimateBricks(d,0,0,penPose{1},0,InitialQ);

    InitialQ = d.model.getpos();
    AnimateBricks(d,0,0,penPoseIntermediary{1},penObj,InitialQ);

    InitialQ = [-1.5708    1.0472    1.0472    1.0472         0];
    AnimateBricks(d,0,0,penPoseFinal{1},penObj,InitialQ);

    % Make Dobot pick up ruler
    InitialQ = [0    0.7854    0.7854    1.5708         0];
    AnimateBricks(d,0,0,rulerPose{1},0,InitialQ);

    InitialQ = d.model.getpos();
    AnimateBricks(d,0,0,rulerPoseIntermediary{1},rulerObj,InitialQ);

    InitialQ = [-1.5708    1.0472    1.0472    1.0472         0];
    AnimateBricks(d,0,0,rulerPoseFinal{1},rulerObj,InitialQ);
