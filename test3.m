...
q0 = [0 107*pi/180 -64.7*pi/180 11.2*pi/180 -47.8*pi/180 0 0];              % Initial guess for joint angles
% q0 = [0.8 -16.9*pi/180 -166*pi/180 87.5*pi/180 -202*pi/180 0 0];
...
for k = 1:length(qMatrix)
    % p560.plot(qMatrix(k,:),'trail','r-','scale',0.5,'nowrist','nojoints','notiles','noarrow')
    p560.animate(qMatrix(k,:));
    drawnow();
    % pause(0.1);
end
